...
export default function About() {
  ...
  const fetchStats = async () => {
    try {
      ...
      setCertificateCount(certificates?.length || 0);
    } catch {
      setProjectCount(0);
  ...
  const scrollToPortfolioTab = (tab: string) => {
    sessionStorage.setItem("portfolioActiveTab", tab);
    const el = document.getElementById("portfolio");
    if (el) {
      el.scrollIntoView({ behavior: "smooth" });
    }
        ...
        >
          {stats.map((item, i) => (
            <motion.div
              ...
              onClick={() => scrollToPortfolioTab(item.tab)}
              style={{
                position: "relative",
                padding: 18,
                borderRadius: 16,
